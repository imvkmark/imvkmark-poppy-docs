...
<p>下面是示例 :</p>
<div class="language-text line-numbers-mode" data-ext="text"><pre v-pre class="language-text"><code>$options = array(    'validate_all' => true,    'return_type' => 'both');list($validate, $allValidations) = $user->ability(    array('admin', 'owner'),    array('create-post', 'edit-user'),    $options);var_dump($validate);// bool(false)var_dump($allValidations);// array(4) {//     ['role'] => bool(true)//     ['role_2'] => bool(false)//     ['create-post'] => bool(true)//     ['edit-user'] => bool(false)// }
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><p><code v-pre>Rbac</code> 有一个 <code v-pre>ability()</code> 快捷方式来检测当前的用户</p>
...
<div class="language-text line-numbers-mode" data-ext="text"><pre v-pre class="language-text"><code>@role('admin')    &lt;p>This is visible to users with the admin role. Gets translated to    \Rbac::role('admin')&lt;/p>@endrole@permission('manage-admins')    &lt;p>This is visible to users with the given permissions. Gets translated to    \Rbac::capable('manage-admins'). The @can directive is already taken by core    laravel authorization package, hence the @permission directive instead.&lt;/p>@endpermission@ability('admin,owner', 'create-post,edit-user')    &lt;p>This is visible to users with the given abilities. Gets translated to    \Rbac::ability('admin,owner', 'create-post,edit-user')&lt;/p>@endability
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><h3 id="middleware-中间件" tabindex="-1"><a class="header-anchor" href="#middleware-中间件" aria-hidden="true">#</a> Middleware 中间件</h3>
<p>你可以使用中间件来过滤路由或者是一组路由, 通过权限或者是角色</p>
<div class="language-text line-numbers-mode" data-ext="text"><pre v-pre class="language-text"><code>Route::group(['prefix' => 'admin', 'middleware' => ['role:admin']], function() {    Route::get('/', 'AdminController@welcome');    Route::get('/manage', ['middleware' => ['permission:manage-admins'], 'uses' => 'AdminController@manageAdmins']);});
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><p>It is possible to use pipe symbol as <em>OR</em> operator: 也可以使用管道符号 <code v-pre>|</code> 来替代 <code v-pre>OR</code> 选项</p>
...
<div class="language-text line-numbers-mode" data-ext="text"><pre v-pre class="language-text"><code>// only users with roles that have the 'manage_posts' permission will be able to access any route within admin/postRbac::routeNeedsPermission('admin/post*', 'create-post');// only owners will have access to routes within admin/advancedRbac::routeNeedsRole('admin/advanced*', 'owner');// optionally the second parameter can be an array of permissions or roles// user would need to match all roles or permissions for that routeRbac::routeNeedsPermission('admin/post*', array('create-post', 'edit-comment'));Rbac::routeNeedsRole('admin/advanced*', array('owner','writer'));
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><p>以上的方法都接受第三个参数, 如果第三个参数传递为空则返回 <code v-pre>App::abort(403)</code>, 否则返回其他的输入值</p>
<p>所以你可以如下使用 :</p>
...
<div class="language-text line-numbers-mode" data-ext="text"><pre v-pre class="language-text"><code>// if a user has 'create-post', 'edit-comment', or both they will have accessRbac::routeNeedsPermission('admin/post*', array('create-post', 'edit-comment'), null, false);// if a user is a member of 'owner', 'writer', or both they will have accessRbac::routeNeedsRole('admin/advanced*', array('owner','writer'), null, false);// if a user is a member of 'owner', 'writer', or both, or user has 'create-post', 'edit-comment' they will have access// if the 4th parameter is true then the user must be a member of Role and must have PermissionRbac::routeNeedsRoleOrPermission(    'admin/advanced*',    array('owner', 'writer'),    array('create-post', 'edit-comment'),    null,    false);
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><h3 id="路由过滤" tabindex="-1"><a class="header-anchor" href="#路由过滤" aria-hidden="true">#</a> 路由过滤</h3>
<p><code v-pre>Rbac</code> roles/permissions 也能够很简单的写在路由的过滤器中通过 Facade 进行权限控制</p>
<div class="language-text line-numbers-mode" data-ext="text"><pre v-pre class="language-text"><code>Route::filter('manage_posts', function(){    // check the current user    if (!Rbac::capable('create-post')) {        return Redirect::to('admin');    }});// only users with roles that have the 'manage_posts' permission will be able to access any admin/post routeRoute::when('admin/post*', 'manage_posts');
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><p>过滤一个角色:</p>
<div class="language-text line-numbers-mode" data-ext="text"><pre v-pre class="language-text"><code>Route::filter('owner_role', function(){    // check the current user    if (!Rbac::hasRole('Owner')) {        App::abort(403);    }});// only owners will have access to routes within admin/advancedRoute::when('admin/advanced*', 'owner_role');
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><p>如上所示 <code v-pre>Rbac::hasRole()</code> 和 <code v-pre>Rbac::capable()</code> 检查用户是否已经登陆同时这个用户是否有这个权限来操作这个功能. 如果用户未登录则返回的值一定是 false;</p>
